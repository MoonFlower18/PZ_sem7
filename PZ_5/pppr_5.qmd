...
```{r}
uni_mac <- unique(ds2$BSSID)
uni_mac <- as.data.frame(uni_mac)
uni_mac

counts <- count(ds2, BSSID)
counts <- counts %>% arrange(desc(n))
counts <- counts[-c(1, 2), ]

counts2 <- counts %>% arrange(desc(BSSID)) 
counts2 <- counts2[setdiff(1:nrow(counts2), c(1, 2, 4:23, 25:27, 30:33, 36, 39, 40, 71:73)), ]
counts2 %>% select(BSSID)
```

```{r}
test <- ds2 %>%
  group_by(BSSID) %>%
  summarize(num_unique_macs = n_distinct(Station.MAC)) %>%
  filter(num_unique_macs == 1)
test

test2 <- ds2 %>% filter(grepl("(..:..:..)", BSSID)) %>% select(BSSID)
test2 <- test2 %>% arrange(desc(BSSID))
test2

#no_rand <- ds2 %>% filter(BSSID == "(not associated)") %>%
#  select(Station.MAC, BSSID, Probed.ESSIDs) %>% arrange(desc(Probed.ESSIDs))
